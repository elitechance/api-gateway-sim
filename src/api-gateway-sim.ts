...
class ApiGatewaySim {
  ...
  private initCommander() {
    commander
      ...
      .option(
        '-t, --stage-variables <file>',
        'Default file stage-variables.json file'
      )
  ...
  private getRequestTemplate(method: Method, contentType: string): string {
    ...
    if (!templateValue) {
      return this.getPassThroughTemplate(
        method.integration.passthroughBehavior
      );
    }
  }
  ...
  private processProxyData(
    ...
    if (method.integration.type === 'aws_proxy') {
      requestObject.eventJson.pathParameters = this.getProxyPathParams(
        path,
        request
      );
  ...
  private getMethodResponseByStatusCode(
    method: Method,
    statusCode: number
  ): MethodResponse {
  ...
  private setHeadersByIntegrationResponse(
    integrationResponse: IntegrationResponse,
    method: Method,
    httpResponse: Response
  ) {
    const methodResponse: MethodResponse = this.getMethodResponseByStatusCode(
      method,
    ...
    for (const header of methodResponse.headers) {
      for (const responseParameter of integrationResponse.responseParameters) {
        ...
        const responseHeader =
          integrationResponse.baseHeaderName + '.' + methodResponseHeader;
  ...
  private sendDefaultResponse(
    ...
    if (this._strictCors) {
      this.setHeadersByIntegrationResponse(
        method.integration.defaultResponse,
        method,
        httpResponse
      );
    }
  ...
  private isAuthorizerRequest() {
    if (!this.currentRequest) {
      return false;
    }
    if (!this.currentRequest.eventJson) {
      return false;
    }
    if (!this.currentRequest.eventJson.authorizationToken) {
      return false;
    } else {
      return true;
    }
  ...
  private sendHttpSuccessResponse(
    httpResponse: Response,
    method: Method,
    message: any
  ) {
    if (this.isAuthorizerRequest()) {
  ...
  private processHandlerResponse(
    ...
    if (lambdaResponse.lambdaError) {
      ...
      this.sendHttpErrorResponse(
        httpResponse,
        'Task timed out after ' + this.getLambdaTimeout() + '.00 seconds'
      );
    } else if (lambdaResponse.error) {
      const error = lambdaResponse.error;
      const integrationResponse = method.integration.getResponseByErrorMessage(
        error.message
      );
      if (this._strictCors) {
        this.setHeadersByIntegrationResponse(
  ...
  private loadPackageJson() {
    try {
      ...
      this.errorMessage(
        'Missing package.json, Please this inside your lambda application root directory.'
      );
    }
  }
